class Game {
  ...
  setup() {
    ...
    //Questions-Array
    this.questionArray = [
      "Zooplankton is the name for small aquatic…?",
  ...
  gameOver() {
    ...
    document.getElementById(
      "question-image"
    ).src = `style_assets/game_over_daphnia.gif`;
    ...
    if (totalScore <= 0) {
      document.querySelector(
        ".question-text"
      ).innerText = `${totalScore} from ${possiblePoints} points. You still have a lot of exploring to do!`;
    } else if (totalScore === 1) {
      document.querySelector(
        ".question-text"
      ).innerText = `Nice try! You scored ${totalScore} of ${possiblePoints} points. You still have a lot of exploring to do!`;
    } else if (totalScore < possiblePoints / 3) {
      document.querySelector(
        ".question-text"
      ).innerText = `Nice try! ${totalScore} out of ${possiblePoints} points! Time to explore the underwater-world further!`;
    } else if (totalScore < possiblePoints / 2) {
      document.querySelector(
        ".question-text"
      ).innerText = `Well done! You've obviously been PondDipping before! ${totalScore} out of ${possiblePoints} points!`;
    } else if (totalScore < possiblePoints) {
      document.querySelector(
        ".question-text"
      ).innerText = `Great Job! You really know Attenborough's underwater-world! ${totalScore} out of ${possiblePoints} points!`;
    } else if (totalScore === possiblePoints) {
      document.querySelector(
        ".question-text"
      ).innerText = `Tremendous - You're a true PondDipping-Pro! You scored all ${possiblePoints} points!`;
    } else {
      document.querySelector(
        ".question-text"
      ).innerText = `You scored ${totalScore} out of ${possiblePoints} points.`;
    }
